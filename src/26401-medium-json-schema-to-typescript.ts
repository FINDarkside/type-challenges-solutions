...
type NumberSchema = { type: 'number'; enum?: number[] };
type BooleanSchema = { type: 'boolean' };
type StringSchema = { type: 'string'; enum?: string[] };

type JSONSchema2TS<T extends JsonSchema> = T extends NumberSchema
  ? JSONNumberSchema<T>
  ...
  ? JSONObjectSchema2TS<T>
  : never;

type Simplify<T> = { [K in keyof T]: T[K] };

type JSONObjectSchema2TS<
  T extends ObjectSchema,
  Required = T['required'] extends string[] ? T['required'][number] : never
> = T['properties'] extends Record<string, JsonSchema>
  ? Simplify<
      {
        [K in keyof T['properties'] as K extends Required
          ? never
...
type JSONStringSchema<T extends StringSchema> = T['enum'] extends string[]
  ? T['enum'][number]
  : string;

type JSONNumberSchema<T extends NumberSchema> = T['enum'] extends number[]
  ? T['enum'][number]
  : number;

type JSONArraySchema<T extends ArraySchema> = T['items'] extends JsonSchema
